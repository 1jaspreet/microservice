              ...
              FROM
                  (
                      SELECT
                          ...
                          district
                      FROM
                          pmshri_ann_wrk_pln_bdgt_data pawpbd,
                          pmshri_data pd
                      WHERE
                          state = ${state_id}
...
exports.dashboard = async (req, res) => {
  ...
  left join pmshri_major_component pmc 
        on
    (aa.scheme_id::numeric = pmc.scheme_id
      and aa.major_component_id = pmc.pmshri_major_component_id)`;

    /*     let query = `from 
    (select sum(financial_amount) as financial_amount , scheme_id ,major_component_id 
        from pmshri_ann_wrk_pln_bdgt_data pawpbd 
        where 1 =  1
        ${whereQ}
        group by scheme_id , major_component_id 
      ) aa left join  pmshri_major_component pmc 
      on(aa.scheme_id::numeric= pmc.scheme_id and aa.major_component_id= pmc.pmshri_major_component_id)`; */
